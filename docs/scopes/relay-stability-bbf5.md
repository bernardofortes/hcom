...
|---|---|---|---:|---|---|
| AC-1 | A local event at the 64 KiB serialized boundary is accepted, one byte over returns a typed error with no ID/row, an authenticated imported event is not re-admitted, and instance finalization cannot bypass the same gate. | R1 | 1 | planned | pending |
