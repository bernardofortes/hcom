...
| AC-3 | After an oversize rejection, a following small marker crosses a real MQTT relay and advances the sender cursor only after matching PUBACK; the receiver supplies separate evidence. | R2, R3 | 5 | implemented | The ignored real-MQTT test rejected the oversized public CLI send with no row, then broker-confirmed the next marker and independently observed its exact text and durable relay metadata on the receiver. |
| AC-4 | Legacy UUID/name migrate without changing either value; concurrent first use agrees; restart and ordinary reset preserve both durable files. | R4, R5 | 3 | implemented | Conductor reproduced exact legacy UUID/name migration, same-value restart, concurrent creation/migration, ordinary-reset preservation, and fresh-DB mapping restoration. |
| AC-5 | Conflicting local identity sources fail CLI/worker without fallback; reset-all alone clears both durable files; a remote short-name collision does not rename or stop the local identity. | R4, R5 | 3 | implemented | Typed UUID/name conflicts preserve sources and exit CLI non-zero; worker startup refuses them; reset-all rotates the identity; stale/remote reverse claims are logged, rejected, and cannot remove the restored local mapping. |
| AC-6 | Published state/RPC, suffix stripping, relay status, reset event, TUI, worker, and tests all expose/use the same durable UUID/name after ordinary reset. | R5 | 4 | implemented | A deliberately non-natural durable name was observed unchanged in state/RPC, own-target and suffix logic, CLI/client, reset event, TUI partitioning, and fresh-DB mapping restoration; forbidden-reader search returned empty. |
| AC-7 | Linux and Windows GIGA report `hcom 0.7.24+bbf.5`; GIGA keeps its pre-install short name across daemon restart; both queues are preflight-clean; one marker travels Linux→GIGA and another GIGA→Linux with receiver evidence; relay health is connected; and the reviewed commit is present on the authorized GitHub branch. | R6 | 5 | implemented | Linux `RONI` and Windows `GIGA` run BBF5 from reviewed commit `45f39fb`; durable/legacy UUID hashes match on both; both relays are connected with draining queues; receiver evidence confirmed Linux→GIGA marker and GIGA→RONI request/ack. |
...
| 2 | Caller-controlled message and RPC producers receive truthful actionable oversize failures while `events` still delivers a bounded page · R2 · AC-2 | Blocks 1/3 plus changed existing integration proof under Block 8; roots `src/shared/constants.rs`, `src/shared/mod.rs`, `src/messages.rs`, `src/commands/`, `src/core/`, `src/relay/`, `tests/cli_smoke.rs` · high · 3 blocks/3 clusters/1 AC/7 canonical-module paths; owner waived the block-count split trigger on 2026-08-10 because separating the CLI proof would make the slice non-vertical | `cargo test --locked oversized_message -- --nocapture`; `cargo test --locked oversized_rpc_result -- --nocapture`; `cargo test --locked remote_events_with_event_budget -- --nocapture`; `cargo test --locked --test cli_smoke oversized_send -- --exact --nocapture` from repository root → CLI exit/text/no-row, bounded central RPC error, and truncated-delivered `events` page · writer then conductor | closed · boundary `87f30b8` · writer tasks `ses_0132688fdffegREC35yuQ4sNc4`, `ses_0130f300affe6TRRogJVtMcf9N` · reviewer tasks `ses_0131cec90ffeLgf5ua0kjPcXA1`, `ses_0130b4fccffetKI2bQE5yc0zRO` · conductor candidate validation 7/7 passed | commit `911313b` · ACCEPT after fix round 1 · prior Medium closed; accepted Low observations unchanged |
| 3 | Legacy identity migrates atomically and reset/conflict/collision boundaries preserve the chosen local identity · R4, R5 · AC-4, AC-5 | Blocks 2/3 plus existing CLI integration proof under Block 8; durable authority + migration/locking + reset semantics + collision initialization + typed conflict exit; roots `src/paths.rs`, `src/commands/reset_ops.rs`, `src/relay/`, `src/commands/relay.rs`, `tests/cli_smoke.rs` · normal · 3 blocks/4 clusters/2 ACs/5 canonical-module paths; owner approved the minimal CLI boundary amendment on 2026-08-10 because AC-5 already assigns the conflict exit to this slice | `cargo test --locked durable_device_identity -- --nocapture`; `cargo test --locked durable_device_identity_conflict -- --nocapture`; `cargo test --locked durable_identity_reset -- --nocapture`; `cargo test --locked remote_short_name_collision -- --nocapture`; `cargo test --locked --test cli_smoke relay_status_fails_on_durable_identity_conflict -- --exact --nocapture` from repository root → migration/concurrency/conflict/reset/collision and typed CLI exit proof · writer then conductor | closed · boundary `352edc2` · writer tasks `ses_012f9e951ffemQSaFXL1q4WeX3`, `ses_012cab4d7ffeuT0EV3msQo7hjx` · reviewer tasks `ses_012ea00f1ffevutRiYgzy7f488`, `ses_012c3b148ffer7wqT93xRxZIVv` | commit `7e69240` · ACCEPT after fix round 1 · conductor proofs 9/9; 2,237 tests passed/0 failed/16 ignored; every `just ci` step passed, with the public-broker roundtrip passing separately in 159 s after transient broker timeouts |
| 4 | Every own-device reader uses the canonical durable identity · R5 · AC-6 | Blocks 3/7; relay runtime propagation + reset/TUI propagation; roots `src/relay/`, `src/commands/relay.rs`, `src/db/mod.rs`, `src/tui/db.rs` · normal · 2 blocks/2 clusters/1 AC/4 canonical-module paths; no split trigger | `cargo test --locked durable_identity_consumers -- --nocapture`; `cargo test --locked durable_identity_after_reset -- --nocapture` from repository root → actual canonical value across state/RPC/suffix/CLI/reset/TUI/worker · writer then conductor | closed · boundary `a541d06` · writer task `ses_012b09e05ffeAakDlqdcQQ2ttA` · reviewer task `ses_0129fd7feffevL5pp3QcQ4aDYW` | commit `94831c7` · ACCEPT · conductor proofs 7/7; 2,250 tests passed/0 failed/16 ignored; `just ci` 9 passed/1 skipped in 349 s |
| 5 | One BBF5 artifact passes integrated real-relay and native Windows ship proof · R3, R6 · AC-3, AC-7 | Block 8; real relay regression + version metadata + Linux/Windows install/rollback evidence + authorized branch push; roots `tests/`, `Cargo.toml`, `Cargo.lock`, `pyproject.toml` · normal · 1 block/4 clusters/2 ACs/4 canonical-module paths; no split trigger | `cargo test --locked --test test_relay_roundtrip oversized_rejection_does_not_block_following_event -- --exact --ignored --nocapture --test-threads=1`; `cargo run --locked -- --version` from repository root → real MQTT follow-up and BBF5 version; native GIGA commands resolved in the slice packet from architecture's Windows procedure · writer then conductor | closed · boundary `f82d55f` · writer task `ses_0126f5662ffeGaYa0RcGMCIz0z` · reviewer task `ses_012599149ffe75ZyyIXlmjGUUB` | commit `45f39fb` · ACCEPT · 2,250 tests passed/0 failed/17 ignored; new MQTT proof passed; legacy 14-phase relay proof passed in 172.83 s; native Linux/Windows ship and bidirectional private-broker evidence passed |
...
- Verified recovery point: Linux `/home/ubuntu/.local/bin/hcom-bbf4-20260810_215608` SHA-256 `3161558c8cd6d14d88f2883065b611407f21c3b8da93b0b89bbfbfe4afe0bd8f`; Windows `C:\Users\berna\.local\bin\hcom.exe.bbf4-rollback.bak` SHA-256 `2EE6FBE9B91D239497BC90D49D04662BBDE015B4E523A37F0B55F033C499351C`, byte-identical to the retained BBF4 running copy.
- Rollback compatibility: verified on both hosts. BBF5 preserved matching durable/legacy UUID files and the names `RONI`/`GIGA`; the retained BBF4 binaries report `0.7.24+bbf.4` and can read the unchanged schema/protocol/configuration.
- Deployed revision: reviewed product commit `45f39fbf55d7157cd5d6af8ed6a68802ad81a7cf`, installed as `0.7.24+bbf.5` on Linux RONI and Windows GIGA and present on `origin/fix/relay-stability-bbf5`.
- Health and documented production-safe smoke: both private-broker relays connected after daemon restart; Linux reported `Queued: up to date`; Windows cursor advanced `162→181` and validation queue drained without stationary backlog; GIGA received `BBF5-LINUX-TO-GIGA-20260810T2205Z`, and RONI received/responded to GIGA's `Fluminense` request.
...
- Slice 2 close evidence: product commit `911313b`; fresh reviewer task `ses_0130b4fccffetKI2bQE5yc0zRO` returned ACCEPT after fix round 1; conductor candidate validation passed all 7 focused/changed-test commands; `just ci` passed 9 steps with 1 skipped in 342 seconds. The Slices row remains `reviewed` because its oversized Markdown row cannot be safely rewritten by the available patch transport; this exception is its durable closed-state evidence.
- Owner amendment on 2026-08-10 after Slice 3 review: add `src/commands/relay.rs` and existing `tests/cli_smoke.rs` coverage solely so AC-5's typed local-conflict failure exits non-zero instead of rendering `?` or using an empty own UUID. Slice 4 retains every other presentation and canonical-consumer migration.
- Slice 3 close evidence: product commit `7e69240`; fresh focused reviewer task `ses_012c3b148ffer7wqT93xRxZIVv` returned ACCEPT after fix round 1 closed both prior Mediums; conductor candidate validation passed all 9 packet commands and the complete Rust suite passed 2,237/0/16. `just ci`'s non-relay steps passed twice; the unchanged real public-MQTT roundtrip passed end-to-end once in 159 seconds between two broker-timing failures (one backlog wait and one 5-second ephemeral RPC PUBACK), so no single combined invocation was wholly green. Slice 5 retains final private-broker and physical Linux/Windows delivery proof.
- Slice 3 accepted Low residuals: a rare reset-all identity-file removal failure is logged but does not yet change the command exit code; identity restoration failure can suppress the reset event; transitional `read_device_uuid` opens and writes through the identity transaction until Slice 4 removes it; thread-level locking is proved locally while native Windows/process-level proof remains in Slice 5.
- Slice 4 close evidence: product commit `94831c7`; fresh reviewer task `ses_0129fd7feffevL5pp3QcQ4aDYW` returned ACCEPT; conductor validation passed all 7 packet filters, the complete Rust suite passed 2,250/0/16, forbidden own-reader search returned empty, and clean-target `just ci` passed 9 steps with 1 skipped in 349 seconds including the real public-MQTT roundtrip in 128 seconds. `device_short_id_for_db` has zero production callers and the dead `.tmp/device_uuid`/KV `device_uuid` readers are removed.
- Slice 4 accepted Low residuals: `relay off --all` now stops before local disable when a peer-broadcast setup error occurs; a conflicting identity also blocks `relay off` while direct config disable remains available; persistent TUI identity errors retry initialization each refresh and TUI schema initialization can own future archive behavior; reset-all intentionally records only `relay_local_reset_ts` when no identity remains; client identity accessors currently serve tests/future internal use.
- Slice 5 close evidence: product commit `45f39fb`; fresh reviewer task `ses_012599149ffe75ZyyIXlmjGUUB` returned ACCEPT after running the complete Rust suite at 2,250/0/17. Conductor proof reproduced the oversized-rejection follow-up marker in 29.46 seconds and version/identity/oversize filters; the combined public-broker gate's legacy test hit one transient kill PUBACK timeout after all earlier phases, then the unchanged legacy test passed all 14 phases with pinned mocks in 172.83 seconds.
- Ship evidence: reviewed branch push resolved `origin/fix/relay-stability-bbf5` to `45f39fb`; Linux installed hash `5f7ef132d722d40abec76cefd392dca5cf884adfe560db0a91c7fb6e90492bee`, migrated the exact legacy UUID, retained `RONI`, and reconnected after PVE3/MQTT recovery. Windows built with Rust/Cargo 1.97.1 and the official package smoke, installed BBF5 at `C:\Users\berna\.local\bin\hcom.exe`, retained `GIGA`, proved equal durable/legacy UUID hashes, and restarted worker PID `17188`. Receiver evidence closed both directions; no queue reset, broker change, rollback, force push, PR, or public release occurred during BBF5 ship.
